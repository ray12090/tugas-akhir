            ...
            <div class="p-6 bg-white overflow-hidden shadow-sm sm:rounded-2xl">
                                ...
                                <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                                    ...
                                    <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        <div class="flex items-center space-x-4 justify-center">
                                            ...
                                            <a href="{{ route('detail_biaya_air.edit', $air->id) }}"
                                            ...
                                            <form action="{{ route('detail_biaya_air.destroy', $air->id) }}" method="POST"
                                                onsubmit="return confirmDelete(this, '{{ $air->biaya_air }}');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                ...
                <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
                    ...
                    <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                        Showing
                        <span class="font-semibold text-gray-900 dark:text-white">{{ $detailBiayaAir->firstItem() }}</span>
                        to
                        <span class="font-semibold text-gray-900 dark:text-white">{{ $detailBiayaAir->lastItem() }}</span>
                        of
                        <span class="font-semibold text-gray-900 dark:text-white">{{ $detailBiayaAir->total() }}</span>
                    </span>
                    <ul class="inline-flex items-stretch -space-x-px">
                        <!-- Previous Page Link -->
                        @if ($detailBiayaAir->onFirstPage())
                            <li class="disabled" aria-disabled="true" aria-label="@lang('pagination.previous')">
                                <span class="flex items-center justify-center h-full py-1.5 px-3 ml-0 text-gray-500 bg-white rounded-l-lg border border-gray-300 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                        ...
                        @else
                            <li>
                                <a href="{{ $detailBiayaAir->previousPageUrl() }}" rel="prev"
                        ...
                        <!-- Pagination Elements -->
                        @foreach ($detailBiayaAir->links()->elements[0] as $page => $url)
                            @if ($page == $detailBiayaAir->currentPage())
                                <li>
                        ...
                        <!-- Next Page Link -->
                        @if ($detailBiayaAir->hasMorePages())
                            <li>
                                <a href="{{ $detailBiayaAir->nextPageUrl() }}" rel="next"
                        ...
                        @else
                            <li class="disabled" aria-disabled="true" aria-label="@lang('pagination.next')">
                                <span class="flex items-center justify-center h-full py-1.5 px-3 text-gray-500 bg-white rounded-r-lg border border-gray-300 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                                    <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path fill-rule="evenodd"
                                            d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z"
