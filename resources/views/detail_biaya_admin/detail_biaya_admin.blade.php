                ...
                <div>
                    ...
                    <div
                        ...
                        <div class="w-full md:w-1/2">
                            <form method="GET" action="{{ route('detail_biaya_admin.index') }}"
                        ...
                        <div
                            ...
                            <a href="{{ route('detail_biaya_admin.create') }} " type="button" id="createProductButton"
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    <th class="w-1/4 p-4">
                                        <a
                                            href="{{ route('detail_biaya_admin.index', ['sort_by' => 'biaya_admin', 'sort_order' => $sort_by === 'biaya_admin' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                            {{ __('Biaya Admin') }}
                                            @if ($sort_by === 'biaya_admin')
                                    ...
                                    <th class="w-1/4 p-4 text-center">{{ __('Aksi') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($detailBiayaAdmin as $admin)
                                    <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                                        ...
                                        <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            <div class="flex items-center space-x-4 justify-center">
                                                ...
                                                <a href="{{ route('detail_biaya_admin.edit', $admin->id) }}"
                                                ...
                                                <form action="{{ route('detail_biaya_admin.destroy', $admin->id) }}"
                                                    method="POST"
                                                    onsubmit="return confirmDelete(this, '{{ $admin->biaya_admin }}');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                    ...
                    <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
                        ...
                        <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                            Showing
                            <span
                                class="font-semibold text-gray-900 dark:text-white">{{ $detailBiayaAdmin->firstItem() }}</span>
                            to
                            <span
                                class="font-semibold text-gray-900 dark:text-white">{{ $detailBiayaAdmin->lastItem() }}</span>
                            of
                            <span class="font-semibold text-gray-900 dark:text-white">{{ $detailBiayaAdmin->total() }}</span>
                        </span>
                        <ul class="inline-flex items-stretch -space-x-px">
                            <!-- Previous Page Link -->
                            @if ($detailBiayaAdmin->onFirstPage())
                            ...
                            @else
                                <li>
                                    <a href="{{ $detailBiayaAdmin->previousPageUrl() }}" rel="prev"
                            ...
                            @endif
                            <!-- Pagination Elements -->
                            @foreach ($detailBiayaAdmin->links()->elements[0] as $page => $url)
                                @if ($page == $detailBiayaAdmin->currentPage())
                                    <li>
                            ...
                            @endforeach
                            <!-- Next Page Link -->
                            @if ($detailBiayaAdmin->hasMorePages())
                                <li>
                                    <a href="{{ $detailBiayaAdmin->nextPageUrl() }}" rel="next"
