        ...
        <div class="bg-white overflow-hidden shadow-lg sm:rounded-lg">
            <div class="px-6 pt-6">
                <div class="mb-4 text-xl font-bold text-gray-900 dark:text-white">
                    {{ __('Data Jenis Komplain') }}
                </div>
                <div class="text-gray-500 text-sm font-regular">
                    {{ __('Di bawah merupakan tabel data jenis komplain. Isi tabel ini dapat ditambah, lihat, ubah, dan hapus oleh Tenant Relation.') }}
                </div>
            </div>
            <div
                ...
                <div class="w-full md:w-1/2">
                    <form method="GET" action="{{ route('jenis_komplain.index') }}" class="flex items-center">
                ...
                <div
                    ...
                    <a href = "{{ route('jenis_komplain.create') }} "type="button" id="createProductButton"
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            <th class="p-4">
                                <a
                                    href="{{ route('jenis_komplain.index', ['sort_by' => 'nama_jenis_komplain', 'sort_order' => $sort_by === 'nama_jenis_komplain' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                    {{ __('Jenis Komplain') }}
                                    @if ($sort_by === 'nama_jenis_komplain')
                            ...
                            <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                                ...
                                <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <div class="flex items-center space-x-4 justify-center">
                                        <a href="{{ route('jenis_komplain.edit', $jenis->id) }}"
                                        ...
                                        <a href="{{ route('jenis_komplain.show', $jenis->id) }}"
                                        ...
                                        <form action="{{ route('jenis_komplain.destroy', $jenis->id) }}" method="POST"
                                            onsubmit="return confirmDelete(this, '{{ $jenis->nama_jenis_komplain }}');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
            ...
            <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
                ...
                <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                    Showing
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $jenises->firstItem() }}</span>
                    to
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $jenises->lastItem() }}</span>
                    of
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $jenises->total() }}</span>
                </span>

                <ul class="inline-flex items-stretch -space-x-px">
                    <!-- Previous Page Link -->
                    @if ($jenises->onFirstPage())
                    ...
                    @else
                        <li>
                            <a href="{{ $jenises->previousPageUrl() }}" rel="prev"
                    ...
                    <!-- Pagination Elements -->
                    @foreach ($jenises->links()->elements[0] as $page => $url)
                        @if ($page == $jenises->currentPage())
                            <li>
                    ...
                    <!-- Next Page Link -->
                    @if ($jenises->hasMorePages())
                        <li>
                            <a href="{{ $jenises->nextPageUrl() }}" rel="next"
