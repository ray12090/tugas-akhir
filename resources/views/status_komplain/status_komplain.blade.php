        ...
        <div class="bg-white overflow-hidden shadow-lg sm:rounded-lg">
            <div class="px-6 pt-6">
                <div class="mb-4 text-xl font-bold text-gray-900 dark:text-white">
                    {{ __('Data Status Komplain') }}
                </div>
                <div class="text-gray-500 text-sm font-regular">
                    {{ __('Di bawah merupakan tabel data status komplain. Isi tabel ini dapat ditambah, lihat, ubah, dan hapus oleh Tenant Relation.') }}
                </div>
            </div>
            <div
                ...
                <div class="w-full md:w-1/2">
                    <form method="GET" action="{{ route('status_komplain.index') }}" class="flex items-center">
                ...
                <div
                    ...
                    <a href = "{{ route('status_komplain.create') }} "type="button" id="createProductButton"
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            <th class="p-4">
                                <a
                                    href="{{ route('status_komplain.index', ['sort_by' => 'nama_status_komplain', 'sort_order' => $sort_by === 'nama_status_komplain' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                    {{ __('Status Komplain') }}
                                    @if ($sort_by === 'nama_status_komplain')
                            ...
                            <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                                ...
                                <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <div class="flex items-center space-x-4 justify-center">
                                        <a href="{{ route('status_komplain.edit', $status->id) }}"
                                        ...
                                        <a href="{{ route('status_komplain.show', $status->id) }}"
                                        ...
                                        <form action="{{ route('status_komplain.destroy', $status->id) }}" method="POST"
                                            onsubmit="return confirmDelete(this, '{{ $status->nama_status_komplain }}');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
            ...
            <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
                ...
                <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                    Showing
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $statuses->firstItem() }}</span>
                    to
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $statuses->lastItem() }}</span>
                    of
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $statuses->total() }}</span>
                </span>
                ...
                <ul class="inline-flex items-stretch -space-x-px">
                    ...
                    @else
                        <li>
                            <a href="{{ $statuses->previousPageUrl() }}" rel="prev"
                    ...
                    @if ($statuses->hasMorePages())
                        <li>
                            <a href="{{ $statuses->nextPageUrl() }}" rel="next"
