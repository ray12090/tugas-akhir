        ...
        <div class="bg-white overflow-hidden shadow-lg sm:rounded-lg">
            <div class="px-6 pt-6">
                <div class="mb-4 text-xl font-bold text-gray-900 dark:text-white">
                    {{ __('Data Kewarganegaraan') }}
                </div>
                <div class="text-gray-500 text-sm font-regular">
                    {{ __('Di bawah merupakan tabel data kewarganegaraan. Isi tabel ini dapat ditambah, lihat, ubah, dan hapus oleh Tenant Relation.') }}
                </div>
            </div>
            <div
                ...
                <div class="w-full md:w-1/2">
                    <form method="GET" action="{{ route('detail_kewarganegaraan.index') }}" class="flex items-center">
                ...
                <div
                    ...
                    <a href = "{{ route('detail_kewarganegaraan.create') }} "type="button" id="createProductButton"
                        ...
                        {{ __('Tambah Data Kewarganegaraan') }}
                    </a>
                </div>
            </div>
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            <th class="p-4">
                                <a
                                    href="{{ route('detail_kewarganegaraan.index', ['sort_by' => 'status_kewarganegaraan', 'sort_order' => $sort_by === 'status_kewarganegaraan' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                    {{ __('Negara') }}
                                    @if ($sort_by === 'status_kewarganegaraan')
                        ...
                        @foreach ($kewarganegaraans as $kewarganegaraan)
                            <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                                <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ $kewarganegaraan->status_kewarganegaraan }}
                                </td>
                                <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <div class="flex items-center space-x-4 justify-center">
                                        <a href="{{ route('detail_kewarganegaraan.edit', $kewarganegaraan->id) }}"
                                        ...
                                        <a href="{{ route('detail_kewarganegaraan.show', $kewarganegaraan->id) }}"
                                        ...
                                        <form action="{{ route('detail_kewarganegaraan.destroy', $kewarganegaraan->id) }}" method="POST"
                                            onsubmit="return confirmDelete(this, '{{ $kewarganegaraan->status_kewarganegaraan }}');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
            ...
            <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
                ...
                <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                    Showing
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $kewarganegaraans->firstItem() }}</span>
                    to
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $kewarganegaraans->lastItem() }}</span>
                    of
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $kewarganegaraans->total() }}</span>
                </span>

                <ul class="inline-flex items-stretch -space-x-px">
                    <!-- Previous Page Link -->
                    @if ($kewarganegaraans->onFirstPage())
                    ...
                    @else
                        <li>
                            <a href="{{ $kewarganegaraans->previousPageUrl() }}" rel="prev"
                    ...
                    <!-- Pagination Elements -->
                    @foreach ($kewarganegaraans->links()->elements[0] as $page => $url)
                        @if ($page == $kewarganegaraans->currentPage())
                            <li>
                    ...
                    <!-- Next Page Link -->
                    @if ($kewarganegaraans->hasMorePages())
                        <li>
                            <a href="{{ $kewarganegaraans->nextPageUrl() }}" rel="next"
