        ...
        <div class="p-6 bg-white overflow-hidden shadow-sm sm:rounded-2xl">
            <div>
                ...
                <div class="text-gray-500 text-sm font-regular">
                    {{ __('Di bawah merupakan tabel akun. Isi tabel ini dapat ditambah, lihat, ubah, dan hapus oleh Admin.') }}
                </div>
            </div>
            <div>
                <div class="relative sm:rounded-lg overflow-hidden">
                    <div
                        ...
                        <div class="w-full md:w-1/2">
                            <form method="GET" action="{{ route('akun.index') }}" class="flex items-center">
                        ...
                        <div
                            ...
                            <a href="{{ route('akun.create') }}" id="createProductButton"
                                class="flex items-center justify-center text-white bg-[#016452] hover:bg-[#014F41] focus:ring-4 focus:outline-none font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-[#016452] focus:ring-[#014F41]">
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead
                                ...
                                <tr>
                                    <th class="w-1/5 p-4">
                                        <a
                                            href="{{ route('akun.index', ['sort_by' => 'id', 'sort_order' => $sort_by === 'id' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                            {{ __('ID') }}
                                            @if ($sort_by === 'id')
                                    ...
                                    <th class="w-1/5 p-4">
                                        <a
                                            href="{{ route('akun.index', ['sort_by' => 'name', 'sort_order' => $sort_by === 'name' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                            {{ __('Nama') }}
                                            @if ($sort_by === 'nama')
                                                <svg class="w-4 h-4 inline" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <th class="w-1/5 p-4">
                                        <a
                                            href="{{ route('akun.index', ['sort_by' => 'email', 'sort_order' => $sort_by === 'email' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                            {{ __('Email') }}
                                            @if ($sort_by === 'email')
                                                <svg class="w-4 h-4 inline" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <th class="w-1/5 p-4">
                                        <a
                                            href="{{ route('akun.index', ['sort_by' => 'tipe_user_id', 'sort_order' => $sort_by === 'tipe_user_id' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                            {{ __('Tipe Akun') }}
                                            @if ($sort_by === 'tipe_user_id')
                                                <svg class="w-4 h-4 inline" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        stroke-width="2"
                                    ...
                                    <th class="w-1/5 p-4 text-center">{{ __('Aksi') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($users as $user)
                                    <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                                        ...
                                        <td
                                            class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            {{ $user->tipeUser->nama_tipe_user }}</td>
                                        <td
                                            class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            <div class="flex items-center space-x-4 justify-center">
                                                <a href="{{ route('akun.edit', $user->id) }}"
                                                    class="py-2 px-3 flex items-center text-sm font-medium text-center text-white bg-[#016452] rounded-lg hover:bg-[#014F41] focus:ring-4 focus:outline-none focus:[#014f415e] dark:bg-primary-600 dark:hover:bg-[#016452] dark:focus:ring-[#014F41]">
                                                ...
                                                <form action="{{ route('akun.destroy', $user->id) }}" method="POST"
                                                    onsubmit="return confirmDelete(event, this, '{{ $user->name }}');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                                                        class="flex items-center text-red-700 hover:text-white border border-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-3 py-2 text-center dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-600 dark:focus:ring-red-900"
                                                        {{ $user->tipe_user_id == 1 ? 'disabled' : '' }}>
                                ...
                                @if ($page == $users->currentPage())
                                    <li>
                                        <a href="#" aria-current="page"
                                            class="flex items-center justify-center text-sm py-2 px-3 leading-tight text-primary-600 bg-primary-50 border border-primary-300 hover:bg-primary-100 hover:text-[#016452] dark:border-gray-700 dark:bg-gray-700 dark:text-white">
                                            {{ $page }}
                                        </a>
                                    </li>
                                @else
                ...
                <script>
                    ...
                    function confirmDelete(event, form, name) {
                        ...
                        deleteModalText.textContent = `{{ __('Hapus Akun ') }}${name}`;
