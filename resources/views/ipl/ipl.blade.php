        ...
        <div class="pb-6">
            ...
            {{-- @include('components.breadcrumbs', [
                'breadcrumbs' => [
                    ...
                    ['title' => 'Data IPL', 'url' => route('ipl.index')]
                ]
            ]) --}}
        </div>
        <div class="p-6 bg-white overflow-hidden shadow-sm sm:rounded-2xl">
            <div>
                ...
                <div class="text-gray-500 text-sm font-regular">
                    {{ __('Di bawah merupakan tabel IPL. Isi tabel ini dapat ditambah, lihat, ubah, dan hapus oleh Finance.') }}
                </div>
            </div>
            <div>
                <div class="relative sm:rounded-lg overflow-hidden">
                    <div
                        ...
                        <div class="w-full md:w-1/2">
                            <form method="GET" action="{{ route('ipl.index') }}" class="flex items-center">
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead
                                ...
                                <tr>
                                    <th class="w-1/6 p-4">
                                        <a
                                            href="{{ route('ipl.index', ['sort_by' => 'nomor_invoice', 'sort_order' => $sort_by === 'nomor_invoice' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                            {{ __('Nomor Invoice') }}
                                            @if ($sort_by === 'nomor_invoice')
                                    ...
                                    <th class="w-1/6 p-4">
                                        <a
                                            href="{{ route('ipl.index', ['sort_by' => 'unit_id', 'sort_order' => $sort_by === 'unit_id' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                            {{ __('Unit') }}
                                            @if ($sort_by === 'unit_id')
                                                <svg class="w-4 h-4 inline" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <th class="w-1/6 p-4">
                                        <a
                                            href="{{ route('ipl.index', ['sort_by' => 'tanggal_invoice', 'sort_order' => $sort_by === 'tanggal_invoice' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                            {{ __('Tanggal Invoice') }}
                                            @if ($sort_by === 'tanggal_invoice')
                                                <svg class="w-4 h-4 inline" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        stroke-width="2"
                                    ...
                                    <th class="w-1/6 p-4">
                                        <a
                                            href="{{ route('ipl.index', ['sort_by' => 'jatuh_tempo', 'sort_order' => $sort_by === 'jatuh_tempo' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                            {{ __('Jatuh Tempo') }}
                                            @if ($sort_by === 'jatuh_tempo')
                                                <svg class="w-4 h-4 inline" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        stroke-width="2"
                                    ...
                                    <th class="w-1/6 p-4">
                                        <a
                                            href="{{ route('ipl.index', ['sort_by' => 'total', 'sort_order' => $sort_by === 'total' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                            {{ __('Total Tagihan') }}
                                            @if ($sort_by === 'total')
                                                <svg class="w-4 h-4 inline" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <th class="w-1/6 p-4">
                                        <a
                                            href="{{ route('ipl.index', ['sort_by' => 'status', 'sort_order' => $sort_by === 'status' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                            {{ __('Status') }}
                                            @if ($sort_by === 'status')
                                                <svg class="w-4 h-4 inline" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <td
                                            ...
                                            <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                <div class="flex items-center space-x-4 justify-center">
                                                    ...
                                                    <a href="{{ route('ipl.show', $ipl->id) }}" class="py-2 px-3 flex items-center text-sm font-medium text-center text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-gray-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
                                                    ...
                                                    <form action="{{ route('ipl.destroy', $ipl->id) }}" method="POST" onsubmit="return confirmDelete(this, '{{ $ipl->nomor_invoice }}');">
                                                        @csrf
                                                        @method('DELETE')
                    ...
                    <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
                        ...
                        <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                            Showing
                            <span
                                class="font-semibold text-gray-900 dark:text-white">{{ $ipls->firstItem() }}</span>
                            to
                            <span
                                class="font-semibold text-gray-900 dark:text-white">{{ $ipls->lastItem() }}</span>
                            of
                            <span class="font-semibold text-gray-900 dark:text-white">{{ $ipls->total() }}</span>
                        </span>
                        <ul class="inline-flex items-stretch -space-x-px">
                            ...
                            @else
                                <li>
                                    <a href="{{ $ipls->previousPageUrl() }}" rel="prev"
                            ...
                            @endif
                            <!-- Pagination Elements -->
                            @foreach ($ipls->links()->elements[0] as $page => $url)
                            ...
                            @endforeach
                            <!-- Next Page Link -->
                            @if ($ipls->hasMorePages())
                                <li>
                                    <a href="{{ $ipls->nextPageUrl() }}" rel="next"
                ...
                <script>
                    ...
                    function confirmDelete(form, nomorLaporan) {
                        ...
                        deleteModalText.textContent = `{{ __('Hapus Tagihan ')}}${nomorLaporan}`;
