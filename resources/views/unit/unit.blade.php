<x-app-layout>
        ...
        <div class="p-6 bg-white overflow-hidden shadow-sm sm:rounded-2xl">
            <div>
                ...
                <div class="text-gray-500 text-sm font-reguler">
                    {{ __('Di bawah merupakan tabel unit. Isi tabel ini dapat ditambah, lihat, ubah, dan hapus oleh Pengelola.') }}
                </div>
            </div>
            <div>
                <div class="relative sm:rounded-lg overflow-hidden">
                    <div class="flex flex-col md:flex-row items-stretch md:items-center md:space-x-3 space-y-3 md:space-y-0 justify-between mx-4 py-4 dark:border-gray-700">
                        <div class="w-full md:w-1/2">
                            <form method="GET" action="{{ route('unit.index') }}" class="flex items-center">
                                <div class="relative w-full">
                                    ...
                                    <input type="text" name="search" id="simple-search" placeholder="{{ __('Cari Unit') }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full pl-10 p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" value="{{ request('search') }}">
                                </div>
                            </form>
                        </div>
                        <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                            <a href = "{{ route('unit.create') }} "type="button"  id="createProductButton" data-modal-toggle="createProductModal" class="flex items-center justify-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-screen text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    <th class="p-4">
                                        <a href="{{ route('unit.index', ['sort_by' => 'nama_tower', 'sort_order' => $sortBy === 'nama_tower' && $sortOrder === 'asc' ? 'desc' : 'asc']) }}">
                                            {{ __('Nama Tower') }}
                                            @if ($sortBy === 'nama_tower')
                                    ...
                                    <th class="p-4">
                                        <a href="{{ route('unit.index', ['sort_by' => 'nama_lantai', 'sort_order' => $sortBy === 'nama_lantai' && $sortOrder === 'asc' ? 'desc' : 'asc']) }}">
                                            {{ __('Nama Lantai') }}
                                    ...
                                    <th class="p-4">
                                        <a href="{{ route('unit.index', ['sort_by' => 'nama_unit', 'sort_order' => $sortBy === 'nama_unit' && $sortOrder === 'asc' ? 'desc' : 'asc']) }}">
                                            {{ __('Nama Unit') }}
                                            @if ($sortBy === 'nama_unit')
                                                <svg class="w-4 h-4 inline" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <tbody>
                                ...
                                <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                                    <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $unit->lantai->tower->nama_tower }}</td>
                                    <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $unit->lantai->nama_lantai }}</td>
                                    <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $unit->nama_unit }}</td>
                                    <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        <div class="flex items-center space-x-4 justify-center">
                                            ...
                                            <form action="{{ route('unit.destroy', $unit->id) }}"
                                                method="POST"
                                                onsubmit="return confirmDelete(this, '{{ $unit->nama_unit }}');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                    ...
                    <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
                        ...
                        <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                            Showing
                            <span
                                class="font-semibold text-gray-900 dark:text-white">{{ $units->firstItem() }}</span>
                            to
                            <span
                                class="font-semibold text-gray-900 dark:text-white">{{ $units->lastItem() }}</span>
                            of
                            <span
                                class="font-semibold text-gray-900 dark:text-white">{{ $units->total() }}</span>
                        </span>
                        ...
                        <ul class="inline-flex items-stretch -space-x-px">
                            ...
                            @else
                                <li>
                                    <a href="{{ $units->previousPageUrl() }}" rel="prev"
                            ...
                            @if ($units->hasMorePages())
                                <li>
                                    <a href="{{ $units->nextPageUrl() }}" rel="next"
    ...
    <script>
        ...
        function confirmDelete(form, namaPenghuni) {
            ...
            deleteModalText.textContent = `{{ __('Hapus Data Unit ') }}${namaPenghuni}`;
