<x-app-layout>
                            ...
                            <div class="grid gap-4 sm:grid-cols-4 sm:gap-6">
                                <div class="grid gap-4 sm:col-span-2 sm:grid-cols-4 sm:gap-6">
                                        ...
                                        <select name="unit_id" id="unit_id"
                                            ...
                                            @foreach ($units as $unit)
                                                <option value="{{ $unit->id }}"
                                                    {{ $pemilik->unit_id == $unit->id ? 'selected' : '' }}>
                                    ...
                                    <div class="sm:col-span-3">
                                        <label for="nik"
                                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('NIK') }}</label>
                                        <div class="relative">
                                        ...
                                        <select name="agama_id" id="agama_id"
                                            ...
                                            @foreach ($detailAgamas as $detailAgama)
                                                <option value="{{ $detailAgama->id }}"
                                                    {{ $pemilik->agama_id == $detailAgama->id ? 'selected' : '' }}>
                                        ...
                                        <select name="perkawinan_id" id="perkawinan_id"
                                            ...
                                            @foreach ($detailPerkawinans as $detailPerkawinan)
                                                <option value="{{ $detailPerkawinan->id }}"
                                                    {{ $pemilik->perkawinan_id == $detailPerkawinan->id ? 'selected' : '' }}>
                                        ...
                                        <select name="user_id" id="user_id"
                                            ...
                                            @foreach ($users as $user)
                                                <option value="{{ $user->id }}"
                                                    {{ $pemilik->user_id == $user->id ? 'selected' : '' }}>
                                                    {{ $user->email }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <div class="grid gap-4 sm:col-span-1 sm:grid-cols-2 sm:gap-6">
                                    <div class="sm:col-span-1">
                                        ...
                                        <button id="dropdownSearchButtonTempatLahir"
                                            ...
                                            type="button">{{ __('Kabupaten/Kota') }}
                                            <svg class="w-2.5 h-2.5 ms-3 items-end" aria-hidden="true"
                                                    ...
                                                    <li>
                                                        <div
                                                            ...
                                                            <input id="tempat_lahir_id_{{ $kota->id }}"
                                                                ...
                                                                {{ $pemilik->tempat_lahir_id == $kota->id ? 'checked' : '' }}
                                    ...
                                    <div class="sm:col-span-1">
                                        ...
                                        <select name="jenis_kelamin" id="jenis_kelamin"
                                            ...
                                            <option value="Laki-laki"
                                                {{ $pemilik->jenis_kelamin == 'Laki-laki' ? 'selected' : '' }}>
                                                {{ __('Laki-laki') }}</option>
                                            <option value="Perempuan"
                                                {{ $pemilik->jenis_kelamin == 'Perempuan' ? 'selected' : '' }}>
                                                {{ __('Perempuan') }}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="sm:col-span-1">
                                    ...
                                    <div class="relative max-w">
                                        ...
                                        <input id="tanggal_lahir" name="tanggal_lahir" type="text"
                                            value="{{ old('tanggal_lahir', $pemilik->tanggal_lahir) }}" datepicker
                                        ...
                                        <select name="alamat_provinsi_id" id="alamat_provinsi_id"
                                            ...
                                            @foreach ($detailAlamatProvinsi as $provinsi)
                                                <option value="{{ $provinsi->id }}"
                                                    {{ $pemilik->alamat_provinsi_id == $provinsi->id ? 'selected' : '' }}>
                                ...
                                <div class="sm:col-span-1">
                                    ...
                                    <div class="relative max-w">
                                        ...
                                        @foreach ($pemilik->unit as $huni)
                                        <input id="awal_huni" name="awal_huni" type="text"
                                            value="{{ $huni->pivot->awal_huni }}" datepicker
                                            datepicker-format="yyyy-mm-dd" datepicker-buttons
                                            datepicker-autoselect-today
                                ...
                                <div class="sm:col-span-1">
                                    ...
                                    <div class="relative max-w">
                                        ...
                                        @foreach ($pemilik->unit as $huni)
                                        <input id="akhir_huni" name="akhir_huni" type="text"
                                            value="{{ $huni->pivot->akhir_huni }}" datepicker
                                            datepicker-format="yyyy-mm-dd" datepicker-buttons
                                            datepicker-autoselect-today
                                            datepicker-orientation="{auto}"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            // Fetching and updating dependent dropdowns for Wilayah
                ...
                if (provinsiId) {
                    fetch(`/api/get-kabupaten/${provinsiId}`)
                        ...
                        .catch(error => {
                            ...
                            kabupatenSelect.innerHTML = '<option value="">{{ __('Error fetching data') }}</option>';
                            kabupatenSelect.disabled = true;
                        });
                } else {
                    resetDropdown(kabupatenSelect, 'Pilih Kabupaten');
                    resetDropdown(kecamatanSelect, 'Pilih Kecamatan');
                    resetDropdown(kelurahanSelect, 'Pilih Kelurahan');
                }
            });
                ...
                if (kabupatenId) {
                    fetch(`/api/get-kecamatan/${kabupatenId}`)
                        ...
                        .catch(error => {
                            ...
                            kecamatanSelect.innerHTML = '<option value="">{{ __('Error fetching data') }}</option>';
                ...
                if (kecamatanId) {
                    fetch(`/api/get-kelurahan/${kecamatanId}`)
                        ...
                        .catch(error => {
                            ...
                            kelurahanSelect.innerHTML = '<option value="">{{ __('Error fetching data') }}</option>';
