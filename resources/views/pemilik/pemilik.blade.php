        ...
        <div class="p-6 bg-white overflow-hidden shadow-sm sm:rounded-2xl">
            <div>
                ...
                <div class="text-gray-500 text-sm font-regular">
                    {{ __('Di bawah merupakan tabel pemilik. Isi tabel ini dapat ditambah, lihat, ubah, dan hapus oleh Tenant Relation.') }}
                </div>
            </div>
            <div>
                <div class="relative sm:rounded-lg overflow-hidden">
                    <div
                        ...
                        <div class="w-full md:w-1/2">
                            <form method="GET" action="{{ route('pemilik.index') }}" class="flex items-center">
                        ...
                        <div
                            ...
                            <a href = "{{ route('pemilik.create') }} "type="button" id="createProductButton"
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead
                                ...
                                <tr>
                                    <th class="w-1/6 p-4">
                                        <a
                                            href="{{ route('pemilik.index', ['sort_by' => 'nik', 'sort_order' => $sort_by === 'nik' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                            {{ __('NIK') }}
                                            @if ($sort_by === 'nik')
                                    ...
                                    <th class="w-1/6 p-4">
                                        <a
                                            href="{{ route('pemilik.index', ['sort_by' => 'unit', 'sort_order' => $sort_by === 'unit' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                            {{ __('Unit') }}
                                            @if ($sort_by === 'unit')
                                                <svg class="w-4 h-4 inline" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <th class="w-1/6 p-4">
                                        <a
                                            href="{{ route('pemilik.index', ['sort_by' => 'warga_negara_id', 'sort_order' => $sort_by === 'warga_negara_id' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                    ...
                                    <th class="w-1/6 p-4">
                                        <a
                                            href="{{ route('pemilik.index', ['sort_by' => 'agama_id', 'sort_order' => $sort_by === 'agama_id' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                            {{ __('Agama') }}
                                            @if ($sort_by === 'agama_id')
                                                <svg class="w-4 h-4 inline" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <th class="w-1/6 p-4">
                                        <a
                                            href="{{ route('pemilik.index', ['sort_by' => 'perkawinan_id', 'sort_order' => $sort_by === 'perkawinan_id' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                    ...
                                    <th class="w-1/6 p-4">
                                        <a
                                            href="{{ route('pemilik.index', ['sort_by' => 'no_hp', 'sort_order' => $sort_by === 'no_hp' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                            {{ __('No. HP') }}
                                            @if ($sort_by === 'no_hp')
                                                <svg class="w-4 h-4 inline" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <th class="w-1/6 p-4">
                                        <a
                                            href="{{ route('pemilik.index', ['sort_by' => 'tempat_lahir', 'sort_order' => $sort_by === 'tempat_lahir' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                            {{ __('Tempat Lahir') }}
                                    ...
                                    <th class="w-1/6 p-4">
                                        <a
                                            href="{{ route('pemilik.index', ['sort_by' => 'tanggal_lahir', 'sort_order' => $sort_by === 'tanggal_lahir' && $sort_order === 'asc' ? 'desc' : 'asc']) }}">
                                            {{ __('Tanggal Lahir') }}
                                        ...
                                        <td
                                            ...
                                            <div class="flex items-center space-x-4 justify-center">
                                                ...
                                                <form action="{{ route('pemilik.destroy', $pemilik->id) }}"
                                                    method="POST"
                                                    onsubmit="return confirmDelete(this, '{{ $pemilik->nama }}');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                    ...
                    <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
                        ...
                        <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                            ...
                            of
                            <span
                                class="font-semibold text-gray-900 dark:text-white">{{ $pemiliks->total() }}</span>
                        </span>

                        <ul class="inline-flex items-stretch -space-x-px">
                            <!-- Previous Page Link -->
                            @if ($pemiliks->onFirstPage())
                            ...
                            <!-- Pagination Elements -->
                            @foreach ($pemiliks->links()->elements[0] as $page => $url)
                                @if ($page == $pemiliks->currentPage())
                                    <li>
